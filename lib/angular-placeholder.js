...
.directive('placeholder', function($timeout, placeholderSniffer) {
	...
	return {
		...
		link: function(scope, elem, attrs, ngModel) {
			...
			// TODO: try `onselectstart="return false;"` (ondragstart also?)
			if (attrs.ngDisabled) {
			...
			function setUnselectable(newVal) {
				if (elem.hasClass(emptyClassName)) {
					elem.attr('unselectable', newVal ? 'on':'off');
					if (is_pwd) {
			...
			function stylePasswordPlaceholder() {
				clone.val(text)
					...
					.attr('unselectable', elem.attr('unselectable'));
			}
