...
.directive('placeholder', function($timeout, $document, $interpolate, $animate, placeholderSniffer) {
	if (placeholderSniffer.hasPlaceholder()) return {};

	var documentListenersApplied = false,
		supportsAnimatePromises = parseFloat(angular.version.full) >= 1.3,
		requiresHighPriority = parseFloat(angular.version.full) >= 1.2;

	// No native support for attribute placeholder
	return {
		restrict: 'A',
		require: '?ngModel',
		priority: requiresHighPriority ? 110 : 0, // run after ngModel (0) and BOOLEAN_ATTR (100) directives
		link: function(scope, elem, attrs, ngModel) {
			...
			// async update necessary in 1.3+ to account for animation promises
