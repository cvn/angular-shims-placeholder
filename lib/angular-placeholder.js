...
.directive('placeholder', function($timeout, placeholderSniffer) {
	...
	return {
		...
		link: function(scope, elem, attrs, ngModel) {
			...
			// set placeholders unselectable when element is disabled
			if (attrs.ngDisabled) {
				// ngDisabled applies the disabled attribute on a watch, follow it
				scope.$watch(attrs.ngDisabled, setUnselectable);
			} else {
				if (elem.prop('disabled')) {
					setUnselectable(true);
				}
				// handle non-angular changes
				elem.bind('change', function() {
					// match conditionalDefer in updateValue
					conditionalDefer(updateUnselectable);
				});
			}
			// set unselectable correctly when model changes
			// TODO: vs put this in the render function
			if (ngModel) {
				scope.$watch(attrs.ngModel, updateUnselectable);
			}
			...
			function updateUnselectable() {
				setUnselectable(elem.prop('disabled'));
			}

			function setUnselectable(isDisabled) {
				// unseletable is an IE8/9 attribute that prevents text selection
				// TODO: vs `onselectstart="return false;"` (ondragstart?)
				var isEmpty = elem.hasClass(emptyClassName);
				setAttrUnselectable(elem, isDisabled && isEmpty);
				if (is_pwd && isEmpty) {
					showPasswordPlaceholder();
				}
			}
			...
			function stylePasswordPlaceholder() {
				...
				setAttrUnselectable(clone, elem.attr('unselectable'));
			}
